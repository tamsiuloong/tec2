...
public class CmsSiteAccessDaoImpl extends
	...
	public List<Object[]> statisticToday(Integer siteId,String area){
		...
		f.append(" from CmsSiteAccess bean where bean.site.id=:siteId ").setParam("siteId", siteId);

		if(StringUtils.isNotBlank(area)){
			f.append(" and bean.area=:area").setParam("area", area);
		}
		//趋势分析，按时间分组
		f.append(" group by hour(bean.accessTime) order by hour(bean.accessTime) asc");
		return find(f);
	}
