...
public class CmsSiteAccessStatisticDaoImpl extends
	...
	public List<Object[]> statistic(Date begin, Date end,
		...
		if(StringUtils.isNotBlank(statisticValue)){
			f.append(" and bean.statisticColumnValue=:statisticColumnValue")
			.setParam("statisticColumnValue", statisticValue);
		}
	...
	public List<Object[]> statisticTotal(Date begin, Date end,Integer siteId,
		...
		Finder f = Finder.create("select sum(bean.pv) as pv ,"
				...
				+ "(sum(bean.visitSecondAver * bean.visitors)/sum(bean.visitors)) as second"
				+ " from CmsSiteAccessStatistic bean where bean.site.id=:siteId "
				+ "and bean.statisitcType=:statisticType ")
		.setParam("siteId", siteId).setParam("statisticType",statisticType);
		if(begin!=null){
			f.append(" and bean.statisticDate>=:beginDate ").setParam("beginDate", begin);
	...
	public List<Object[]> statisticByTarget(Date begin, Date end,
		...
		hql+=" from CmsSiteAccessStatistic bean where bean.site.id=:siteId "
				+ "and bean.statisitcType=:statisticType";
		Finder f = Finder.create(hql).setParam("siteId", siteId)
				.setParam("statisticType",statisticType);
		if(begin!=null){
			f.append(" and bean.statisticDate>=:beginDate ").setParam("beginDate", begin);
		}
		if(end!=null){
	...
	public List<Object[]> statisticByYear(Integer year,Integer siteId, 
		...
		Finder f = Finder.create("select sum(bean.pv),sum(bean.ip),"
				...
				+ ", month(bean.statisticDate) from CmsSiteAccessStatistic bean "
				+ " where bean.site.id=:siteId "
				+ " and year(bean.statisticDate)=:year "
				+ " and bean.statisitcType=:statisticType ")
		.setParam("siteId", siteId).setParam("year", year)
		.setParam("statisticType",statisticType);
		if(StringUtils.isNotBlank(statisticValue)){
			f.append(" and bean.statisticColumnValue=:statisticColumnValue")
			.setParam("statisticColumnValue", statisticValue);
		}
		f.append(" group by month(bean.statisticDate) ");
		if(orderBy!=null){
			if(orderBy==0){
				f.append(" order by sum(bean.pv) desc");
			}else if(orderBy==1){
				f.append(" order by sum(bean.ip) desc");
			}else if(orderBy==2){
				f.append(" order by (sum(bean.visitSecondAver * bean.visitors)/sum(bean.visitors)) desc");
			}else if(orderBy==3){
				f.append(" order by month(bean.statisticDate) asc");
			}
		}
		return find(f);
	}
