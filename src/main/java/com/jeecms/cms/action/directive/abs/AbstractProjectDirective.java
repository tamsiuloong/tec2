 ...
 * 内容标签基类
 */
public abstract class AbstractProjectDirective implements
	 ...
	 * @throws TemplateException
	 */
	protected int getOpen(Map<String, TemplateModel> params)
	throws TemplateException {
		Integer open = DirectiveUtils.getInt(PARAM_OPEN, params);
		if (open == null) {
			return ContentDoc.ALL;
		} else {
			return open;
		}
	}
	...
	protected Object getData(Map<String, TemplateModel> params, Environment env)
		...
		if (channelIds != null) {
			...
			if (isPage()) {
				...
				return contentMng.getProjectPageByChannelIdsForTag(channelIds,
						typeIds, titleImg, recommend, title,open,attr, orderBy, option,
						pageNo, count);
			} else {
				int first = FrontUtils.getFirst(params);
				return contentMng.getProjectListByChannelIdsForTag(channelIds,
						typeIds, titleImg, recommend, title,open, attr,orderBy, option,
						first, count);
			}
		}
